...
const ProgressBar = ({ progress, isPlaying, onClick }: { 
  ...
  return (
    ...
    >
      ...
      {/* 进度条 */}
      <div 
        className="absolute inset-y-0 left-0 bg-black/30 dark:bg-white/30 rounded-full transition-all"
        style={{ width: `${progress}%` }}
      />

      {/* 指示器 */}
      <div 
        className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 bg-black/30 dark:bg-white/30 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
        style={{ left: `${progress}%` }}
      />
    </div>
  );
