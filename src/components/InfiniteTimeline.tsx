...
export default function InfiniteTimeline({ initialPosts, total, onLoadMore }: TimelineProps) {
  ...
  return (
    <div className="relative">
      {/* 连续的时间线 */}
      <motion.div 
        className="absolute left-[11px] top-2 bottom-0 w-[1.5px] bg-black/[0.07] dark:bg-white/[0.07] rounded-full z-0"
        initial={{ height: 0 }}
        animate={{ height: "auto" }}
      ...
      {/* 加载更多区域 */}
      <div ref={loadMoreRef} className="min-h-[100px] flex items-center justify-center">
        {isLoading && (
          <motion.div 
            ...
            exit={{ opacity: 0 }}
          >
            <div className="w-2 h-2 bg-black/40 dark:bg-white/40 rounded-full animate-bounce [animation-delay:-0.3s]" />
            <div className="w-2 h-2 bg-black/40 dark:bg-white/40 rounded-full animate-bounce [animation-delay:-0.15s]" />
            <div className="w-2 h-2 bg-black/40 dark:bg-white/40 rounded-full animate-bounce" />
          </motion.div>
        )}
        {error && (
          <motion.div 
        ...
        {reachedEnd && (
          <motion.div 
            ...
            transition={{ duration: 0.5 }}
          >
            <div className="inline-flex items-center justify-center px-4 py-2 space-x-2 rounded-full">
